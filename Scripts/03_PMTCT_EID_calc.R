...
# DATE:     2021-05-26
# UPDATED:  2024-02-03
#SOURCE: https://github.com/USAID-OHA-SI/agitprop/blob/0ee29e656bd3fc319aa3aa597040baaf2faec182/Scripts/24_test_qtr-hiv-free-babies.R 
...
hiv_free_all %>% 
  ...
  labs(x = NULL, y = NULL, fill = NULL,
       ...
       caption = glue("Source: {msd_source} (including FY15-23)
...
hiv_free_all %>% 
  mutate(label_txt = clean_number(hiv_free),#label_number_si()(hiv_free), 
         label_tot = clean_number(tot_hiv_free)) %>% #label_number_si(0.01)(tot_hiv_free))
  ggplot(aes(x = fiscal_year)) +
  ...
  geom_text(aes(y = hiv_free,label =label_txt),
            size = 12/.pt,
            family = "Source Sans Pro SemiBold", vjust = 1.5, color = "white") +
  geom_text(data = . %>% filter(fiscal_year == curr_fy), aes(y = tot_hiv_free,
                                                             x = curr_fy +1, 
                                                             label = label_tot),
            size = 12/.pt,
